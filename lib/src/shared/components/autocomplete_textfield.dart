...
class AutoCompleteTextField<T> extends StatefulWidget {
  ...
  AutoCompleteTextField({
    ...
    @required
        this.getSuggestionsMethod, //Get suggestions that will be displayed
...
class AutoCompleteTextFieldState<T> extends State<AutoCompleteTextField> {
  ...
  void updateOverlay([String query]) async {
    ...
    filteredSuggestions =
        await getSuggestions(itemSorter, itemFilter, suggestionsAmount, query);
  ...
  Future<List<T>> getSuggestions(
    ...
    List<T> suggestions = await widget.getSuggestionsMethod(query) ?? [];
